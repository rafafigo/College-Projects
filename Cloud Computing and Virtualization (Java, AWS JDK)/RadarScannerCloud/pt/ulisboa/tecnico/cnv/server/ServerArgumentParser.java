...
public class ServerArgumentParser {
  ...
  private void parseValues() {
    ...
    if (this.cmd.hasOption(ServerParameters.OUTPUT_DIR.toString())) {
      String outDirPath =
          this.cmd.getOptionValue(ServerParameters.OUTPUT_DIR.toString()).replace("'", "");
    ...
    this.argValues.put(
        ServerParameters.DEBUG.toString(), cmd.hasOption(ServerParameters.DEBUG.toString()));
  ...
  private void setupCLIOptions() {
    ...
    final Option addressOption =
        new Option(
            ServerParameters.ADDRESS.toString(),
            true,
    ...
    final Option portOption =
        new Option(ServerParameters.PORT.toString(), true, "server listen port (default: 8000).");
    portOption.setRequired(false);
    this.options.addOption(portOption);

    final Option mapsDirectoryOption =
        new Option(
            ServerParameters.MAPS_DIR.toString(),
            true,
    ...
    final Option outputDirOption =
        new Option(
            ServerParameters.OUTPUT_DIR_SHORT.toString(),
            ServerParameters.OUTPUT_DIR.toString(),
            true,
    ...
    final Option debugOption =
        new Option(
            ServerParameters.DEBUG_SHORT.toString(),
            ServerParameters.DEBUG.toString(),
            false,
            "set debug mode.");
