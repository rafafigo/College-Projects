...
node buildTree(double **pts, long nPts) {
    ...
    return createNode(center, radius, buildTree(ptsL, nPtsL), buildTree(ptsR, nPtsR));
}
...
void filterLR(double **pts, long nPts, const double *projPts, const double *center, double **ptsL, double **ptsR, long *nPtsL, long *nPtsR) {
    ...
    for (long i = 0; i < nPts; i++) {
        if (projPts[i * nDims] < center[0]) {
            ptsL[(*nPtsL)++] = pts[i];
        } else if (projPts[i * nDims] == center[0]) {
            if (!seenEqual) {
                seenEqual = true;
                ptsR[(*nPtsR)++] = pts[i];
            } else {
