...
int main(int argc, char *argv[]) {
    ...
    if (fscanf(fp, "%d %ld", &n_dims, &n_nodes) != 2) exit(EXIT_FAILURE);
    if (n_dims < 2) {
    ...
    allocate_tree();
    for (i = 0; i < n_nodes; i++) {
        if (fscanf(fp, "%ld", &node_idx) != 1) exit(EXIT_FAILURE);
        hash_insert(node_idx, i);
        node = &(tree[i]);
        if (fscanf(fp, "%ld %ld %lf", &(node->L), &(node->R), &(node->radius)) != 3) exit(EXIT_FAILURE);
        for (d = 0; d < n_dims; d++)
            if (fscanf(fp, "%lf", &(center[i][d])) != 1) exit(EXIT_FAILURE);
    }
