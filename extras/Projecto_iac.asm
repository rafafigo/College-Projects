...
var_seguinte_b:    STRING 0       ; 0 - apaga o objecto de baixo,  1 - anda um para o lado
var_seguinte_c:    STRING 0       ; 0 - apaga o objecto de cima,  1 - anda um para o lado
...
baixo_pr:;Verifica se vai apagar ou andar para o lado
	...
	CMP R1, 1
	JZ andar_baixo
apagar_baixo:; Vai apagar o objecto de baixo do pixel screen 
	MOV R0, var_apagar_baixo       
	MOV R1,1               
	...
	MOV R0,var_liga_baixo
	MOV R1,0
	MOVB [R0], R1          ; Desativa a permição de apagar o pixel screen
	MOV R0, var_apagar_baixo 
	MOV R1, 0
    MOVB [R0], R1          ; Desativa apagar no pixel screen
	MOV R0, var_seguinte_b
	MOV R1,1
	MOVB [R0], R1          ; Indica que já apagou e que agora vais andar para o lado, no objecto de baixo
	JMP cima_ap
cima_pr:;Verifica se vai apagar ou andar para o lado o objecto de cima
    MOV R0, var_seguinte_c ; endereço que diz-nos se ja foi apagado o objecto de cima
	MOVB R1, [R0]          ; R1: 0 - apagar, 1 - andar para o lado
	CMP R1, 1
	JZ andar_cima
apagar_cima:; Vai apagar o objecto de cima do pixel_screen
	...
	MOV R0,var_liga_cima   
	MOV R1,0
	MOVB [R0],R1           ; Desligado, deixa de permitir interacoes do objecto de cima no pixel screen
	MOV R0, var_apagar_cima
	MOV R1, 0
    MOVB [R0], R1          ; Desativa apagar o objecto de cima
	MOV R0,var_seguinte_c
	MOV R1,1
...
apagar_nin:; Vai apagar o ninja do pixel_screen
	...
	JMP fim_de_rot
andar_baixo:; O objecto de baixo vai andar para a esquerda 
    MOV R0, col_i_b             
	...
	MOV R0,var_liga_baixo
	MOV R1, 1
	MOVB [R0],R1           ; Ativa a variavel que permite modificacoes do objecto de baixo no pixel screen
	Call n_a_p_pixel
	MOV R0,var_liga_baixo
	MOV R1, 0
...
andar_cima:; O objecto de cima vai andar para a esquerda
	...
	MOV R0,var_liga_cima
	MOV R1,1
	MOVB [R0],R1           ; Ativa a variavel que permite modificaçoes do pixel_screen
