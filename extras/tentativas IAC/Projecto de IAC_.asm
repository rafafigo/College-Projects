...
opr_reset:
    MOV  R9,0           ; faz reset na operacao por clicar em a ou b
ciclo:
...
addi_2:            
	...
	Mov  R1, R6         ; R1 guarda R6 para depois ser feito o resto na conversão para decimal
    MOV  R5, R6	        ; R5 guarda R6 para depois ser feito a divisão inteira na conversão para decimal
    MOV  R8, 10	
	Div  R5, R8         ; parte inteira em decimal
	MOD  R1, R8         ; resto em decimal
	JMP  disp           ; colocar no display
subbi:
	Mov  R9,2           ; permite operaçao de subtrair
	JMP  ciclo          ; agora vai contar!
subbi_2:
	SUB  R6, R7         ; subtração do numero que está no display
	MOV  R5, R6         ; divisão do novo numero sumado
	Mov  R1, R6
	MOV  R8,10
	Div  R5, R8         ; parte inteira em decimal
	MOD  R1, R8         ; resto em decimal
...
subi_3:				    ; funcao de diminuir valor de display por 3
	...
	MOD  R1, R8         ; resto em decimal
	JMP  disp 
;Colocar valor no display
disp:
    SHL  R5, 4          ; R5 vai 4 bits para o lado
	OR   R5,R1          ; é implementado o segundo valor
    MOV  R8,153         ; comparar valor em decimal, 99d é 153h
	CMP  R5,R8          ; é maior ou igual que 99?
    JGT  disp_fin1
