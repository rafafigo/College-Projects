function setRestaurant1() {
    localStorage.setItem("restaurant","1");
}

function setRestaurant2() {
    localStorage.setItem("restaurant","2");
}

function setRestaurant3() {
    localStorage.setItem("restaurant","3");
}

function setRestaurant4() {
    localStorage.setItem("restaurant","4");
}

function setRestaurant5() {
    localStorage.setItem("restaurant","5");
}

function setRestaurant6() {
    localStorage.setItem("restaurant","6");
}

function setRestaurant7() {
    localStorage.setItem("restaurant","7");
}

function setRestaurant8() {
    localStorage.setItem("restaurant","8");
}

function setRestaurant9() {
    localStorage.setItem("restaurant","9");
}

