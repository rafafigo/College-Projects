...
namespace PuppetMaster {
    class PupExec {
        ...
        public async Task<string> CommandExec(string command) {
            ...
            var res = string.Format("$ {0}{1}", command, Environment.NewLine);
        ...
        private string MapCode(string key, int code) {
            ...
            switch (code) {
                case -1:
                    return string.Format("<{0}>: Crashed{1}", key, Environment.NewLine);
                case 0:
                    return string.Format("<{0}>: Available{1}", key, Environment.NewLine);
                case 1:
                    return string.Format("<{0}>: Freezed{1}", key, Environment.NewLine);
                default:
                    return string.Format("<{0}>: Unknown Code {1}", key, Environment.NewLine);
            }
        }
        ...
        private string GetPCSURL(string URL) {
            return string.Format("http://{0}:10000", Lib.ParseURL(URL)[0]);
        }
    }
}
