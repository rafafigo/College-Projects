...
def e_palavra(pala):
    ...
    if pala in sil_final(): #Se for silaba final entao ja e uma palavra
        val_log = True
        return val_log
    return list_final_sil(pala) #Vai verificar se e uma palavra

"""Onde esta armazenada toda a gramatica"""
def art_def():
    """Artigo definido que e junto num tupple"""
    artigo_def = ('A','O') #Junta A, O num tupple 
    return artigo_def
def vog_pal():
    """Junta ao tupple o tupple com a nova vogal que da a vogal_palavra """
    vogal_palavra = art_def() + ('E',) # Junta os dois Tupples
    return vogal_palavra
def vog():
    """Faz juncao de 2 tupples, num so, dando a vogal"""
    vogal = ('I','U',) + vog_pal()
    return vogal
def dit_pal():
    """Indica nos o ditongo_palavra do dicionario, transformando- o num tupple"""
    ditongo_palavra = ('AI','AO','EU','OU',)
    return ditongo_palavra
def dit():
    """Junta dois tupples, dando um ditongo"""
    ditongo = ('AE','AU','EI','OE','OI','IU',) + dit_pal()
    return ditongo
def pa_vog():
    """Junta dois tupples, formando par_vogais"""
    par_vogais = dit() + ('IA','IO',)
    return par_vogais
def cons_freq():    
    """Junta a consoante_freq num tupple"""
...
def cons_final():
    """Juncao de um tupple com outro, formando consoante_final"""
    consoante_final = ('N','P',) + cons_term()
    return consoante_final
def cons():
    """Junta todas as consoantes num tupple"""
    consoante = ('B','C','D','F','G','H','J','L','M','N','P','Q','R','S','T','V','X','Z',)
    return consoante
def pa_cons():
    """junta todos os par_consoantes num tupple"""
    par_consoantes = ('BR','CR','FR','GR','PR','TR','VR','BL','CL','FL','GL','PL',)
    return par_consoantes
def mono_2():
    """Junta varios tupples num so,sempre que se usa a funcao juntar 2 juntao aos elementos do primeiro os do segundo"""
...
def mono_3():
    """Junta varios tupples num so, existe juntar2 e existe juntar3 que funciona da mesma maneira mas o juntar3 adiciona primeiro todos os elementos do segundo a cada elemento do primeiro e depois junta os elementos do terceiro a todas as hipoteses dos primeiros dois"""
    monossilabo_3 = juntar3(cons(),vog(),cons_term()) + juntar2(cons(),dit()) + juntar2(pa_vog(),cons_term())
    return monossilabo_3
def mono():
    """Junta 3 tupples num so que e o monossilabo"""
    monossilabo = vog_pal() + mono_2() + mono_3()
    return monossilabo
def sil_2():
    """junta varios tupples num so formando a silaba_2"""
    silaba_2 = pa_vog() + juntar2(cons(),vog()) + juntar2(vog(),cons_final())
    return silaba_2
def sil_3():
    """Junta varios tupples num so formando a silaba_3"""
    ...
    return silaba_3
def sil_4():
    """Junta os varios tupples num so, dando a silaba_4"""
...
def sil_5():
    ...
    return silaba_5
def sil_final():
    """Junta 4 tupples formando a silaba_final"""
    silaba_final = mono_2() + mono_3() + sil_4() + sil_5()
    return silaba_final
def sil():
    """Juncao de 5 tupples, dando a silaba"""
...
def juntar2(x_tup1,y_tup2):
    """Funcao que junta a cada elemento de um tupple uma certa string"""    
    tup1_ele = 0  
    tup2_ele = 0
    while tup2_ele<len(y_tup2): # Vai passando por cada um dos seus elementos e juntar ao x_tup1
        while tup1_ele<len(x_tup1): #Passa por todos os seus elementos 
            ...
            if (tup1_ele == 0) and (tup2_ele == 0):
                tup_12 = (juntar_12,) #Transforma num tupple
            else:
                tup_12 = tup_12 + (juntar_12,) #Junta tupple
...
def juntar3(x_tup1,y_tup2,z_tup3):
    """Funcao que junta a cada elemento de um tupple uma certa string e junta de seguida outra"""
    tup1_ele = 0
    tup2_ele = 0
    tup3_ele = 0
        ...
        while tup2_ele<len(y_tup2): # Vai passando por cada um dos seus elementos e juntar ao tup1
            while tup1_ele<len(x_tup1):
                ...
                if (tup1_ele == 0) and (tup2_ele == 0) and (tup3_ele == 0):
                    tup_123 = (juntar_123,) #Transforma a juncao num tupple
                else:
                    tup_123 = tup_123 + (juntar_123,) # junta os tupples
...
def list_final_sil(var_sil_val):
