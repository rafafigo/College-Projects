...
def palavra_potencial_para_cadeia(pala_pot):
    return pala_pot # Retorna a propria string pois nesta TAD e utilizado a string como abstracao 
...
def subconjunto_por_tamanho(conjunto_palavras,num):
    """ Vai nos indicar todas as palavras do conjunto de palavras que possuem 
    um determinado conjunto de letras"""
    lst = []
...
def acrescenta_palavra(conj_pala,pala_pot):
    """ Acrescenta ao conjunto de palavras uma palavra potencial caso esta seja 
    de facto uma palavra potencial e esta nao esta ja contida no conjunto """
...
def e_conjunto_palavras(pot_conj):
    ...
    if not isinstance(pot_conj,list):
        ...
        for i in pot_conj:
            if e_palavra_potencial(i)==False: #Verifica se e uma palavra potencial
                return False
            else:
                if i not in lista:    #Cria uma lista com o conjunto de palavras para verificar que nao tem palavras potenciais repetidas
                    lista = lista + [i]
                else:
                    return False      # Tem palavras repetidas
...
def conjuntos_palavras_iguais(conj_pal1,conj_pal2):
    """ Como os dois conjunto de palavras sao listas com strings, entao e possivel 
    organizar por ordem, caso depois de organizadas sejam iguais entao e true """
    conj_pal1.sort() 
    conj_pal2.sort()
...
def conjunto_palavras_para_cadeia(conj_pal):
    """ Cria o conjunto de palavras primeiramente a partir de um dicionario que 
    e separado numa lista com tuples, que prosteriormente se transforma numa so 
    lista que conforme a sua posicao forma a apresentacao pedida no projecto"""
    dic = {} #Dicionario
    newlist = [] 
    string = ''
    lista = []
    ...
    for i in conj_pal:
        ...
        if tamanho in dic:
            dic[tamanho] = dic[tamanho] + [i]   #Se na key Tamanho ja estiver algum valor e adicionado mais esta palavra que tem necessariamente o mesmo numero de silabas
        else:
...
def cria_jogador(cad_carac):
    """ Criacao do jogador numa lista com 4 variaveis, o nome submetido, a 
    pontuacao e uma lista vazia de palavras validas e uma lita vazia de palavras invalidas """
...
def adiciona_palavra_valida(jogador,palavra_potencial): 
    """ Vai caso sejam submetidos argumentos validos, verificado se a palavra 
    valida colocada ja existe no conjunto e se nao existir muda a pontuacao, caso ja existe nada faz"""
    if not e_palavra_potencial(palavra_potencial) or not e_jogador(jogador): #Verifica que o jogador e mesmo jogador conforme as regras e se a palavra e uma palavra potencial
        raise ValueError('adiciona_palavra_valida:argumentos invalidos.') 
    else:
        conjunto = list(jogador[2]) #Lista antes de se adicionar a nova palavra
        acrescenta_palavra(jogador[2],palavra_potencial) # Se ainda nao estiver na lista de palavras validas a lista vai ficar diferente do conjunto criado
...
def adiciona_palavra_invalida(jogador,palavra_potencial):
    """ Vai caso sejam submetidos argumentos validos, verificado se a palavra 
    invalida colocada ja existe no conjunto e se nao existir muda a pontuacao, 
    caso ja existe nada faz"""
    if  not e_jogador(jogador) or not e_palavra_potencial(palavra_potencial): # Verifica que o jogador e um jogador e a palavra e uma palavra potencial
        raise ValueError('adiciona_palavra_invalida:argumentos invalidos.')
    else:
        conjunto = list(jogador[3]) # Lista de palavras invalidas antes de adicionar nova
        acrescenta_palavra(jogador[3],palavra_potencial) # Acrescenta palavra se a palavra potencial nao estiver ja na lista de palavras invalidas
        if conjunto != jogador[3]: # Significa que tem nova palavra
            tamanho = -palavra_tamanho(palavra_potencial)
...
def e_jogador(valor_un):
    ...
    if isinstance(valor_un,list) and len(valor_un) == 4 and isinstance(jogador_nome(valor_un),str) and isinstance(jogador_pontuacao(valor_un),int) and e_conjunto_palavras(jogador_palavras_validas(valor_un)) and e_conjunto_palavras(jogador_palavras_invalidas(valor_un)): # Testa todas as possibilidades para cada uma das variaveis e para as variaveis num todo
        return True
    else:
        return False
...
def gera_todas_palavras_validas(tupl_let):
    """ Primeiramente gera todas as possiveis palavras, depois fica com todas aquelas 
    que sao validas conforme a gramatica do primeiro projecto e e criado a palavra potencial """
    lst = []
    nova_lst = []
...
def guru_mj(letras):
    ...
    jogadores = jogada_dos_jogadores(letras,jogadores)
