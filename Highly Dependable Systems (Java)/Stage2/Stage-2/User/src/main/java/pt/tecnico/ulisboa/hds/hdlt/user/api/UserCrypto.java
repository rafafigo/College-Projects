...
import pt.tecnico.ulisboa.hds.hdlt.user.session.SessionsManager;
...
public class UserCrypto {
  ...
  public byte[] unsignPayload(String name, byte[] signedPayload) {
    return this.doUnsignPayload(this.pubKeys.get(name), signedPayload);
  }
  ...
  public void checkAuthHmac(Session session, byte[] hmac, byte[] payload) {
  ...
  public void removeSession(String name) {
    this.sessionsManager.removeSession(name);
  }
}
